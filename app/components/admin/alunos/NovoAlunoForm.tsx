  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const payload: NovoAlunoCreated = {
        name: fullName.trim(),
        phone: cleanedPhone,
        link: data?.inviteLink || "https://www.vinnylopescoach.site/login",
      };
      setCreated(payload);
      onCreated?.(payload);
    } catch (err: unknown) {
      ...
      <div className="space-y-3 pt-1">
        <a
          href={`https://wa.me/${created.phone}?text=${encodeURIComponent(
            `Fala ${created.name}! Seu perfil no Coach Vinny está pronto. Acesse este link para criar sua senha e ver seu treino: ${created.link}`,
          )}`}
          target="_blank"
