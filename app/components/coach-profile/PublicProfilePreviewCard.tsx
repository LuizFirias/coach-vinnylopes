...
export function PublicProfilePreviewCard({
  ...
  if (compact) {
    return (
      <div className="rounded-xl border border-border-subtle bg-surface-1 p-3 flex items-center gap-3">
        ...
        {form.specialties.length > 0 && (
          <div className="flex flex-wrap gap-1.5 mt-3">
            {form.specialties.slice(0, 5).map((tag) => (
              <span
                key={tag}
                className="text-[10px] font-medium px-2 py-0.5 rounded bg-surface-3 text-text-secondary border border-border-subtle"
              >
                {tag}
              </span>
            ))}
