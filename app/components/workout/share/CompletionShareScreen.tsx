...
export function CompletionShareScreen({
  ...
  const exportCardAt = async (index: number) => {
    try {
      ...
      const fileName = `treino-${cards[index].id}-${temaAtivo}-${Date.now()}.png`;
      ...
      if (navigator.share && navigator.canShare?.({ files: [file] })) {
        ...
        await navigator.share({
          title: `Treino ${nomeRotina}`,
          text: `Treino concluído — ${durationFormatted} · ${volume.toLocaleString('pt-BR')} kg · Coach Vinny`,
        });
      } else {
  ...
  const saveToGallery = async (index: number) => {
    try {
      setExporting(true);
      const blob = await renderCardBlob(index);
      const fileName = `treino-${cards[index].id}-${Date.now()}.png`;
      const file = new File([blob], fileName, { type: 'image/png' });
