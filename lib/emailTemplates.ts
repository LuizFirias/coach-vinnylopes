...
const DEFAULT_SITE_URL = "https://www.auronfit.com.br";
    ...
    <tr>
      <td style="border-top:1px solid ${BORDER}; padding-top:20px;">
        <p style="margin:0; font-size:11px; color:#444444; line-height:1.7; font-family:${FONT};">
          © ${year} AuronFit · Este é um e-mail automático enviado pela plataforma.
...
function wrapEmailTemplate(
  ...
  const title = options?.title ?? "Auronfit";
              ...
              <table role="presentation" border="0" cellpadding="0" cellspacing="0" width="100%">
                <tr>
                  <td style="padding-bottom:14px;">
                    <img class="email-logo" src="${logoUrl(site)}" alt="Auronfit" width="128" style="display:block; border:0; outline:none; max-width:128px; width:128px; height:auto;" />
                  </td>
                </tr>
              </table>
              ${content}
...
export const getPasswordChangedHtml = (fullName: string, siteUrl: string) => {
  ...
  ${emailBody(
    `Olá ${name}, sua senha da plataforma Auronfit foi redefinida com sucesso. Se foi você quem realizou essa alteração, nenhuma ação adicional é necessária.`,
    "24px"
  )}
...
export const getSubscriptionConfirmedHtml = (
  ...
  ${emailBody(
    `Olá Coach ${name}, o pagamento da sua assinatura Auronfit foi processado com sucesso. Seu acesso ao painel está liberado.`
  )}
  ${emailKeyValueTable([
...
export const getSubscriptionPaymentFailedHtml = (fullName: string, updateUrl: string) => {
  ...
  ${emailBody(
    `Olá ${name}, não conseguimos processar a cobrança da sua assinatura Auronfit. Atualize seus dados de pagamento para manter o acesso ao painel.`
  )}
...
export const getPasswordResetNotificationHtml = (fullName: string, siteUrl?: string) => {
  ...
  ${emailBody(`Olá ${name}, sua senha do Auronfit foi alterada com sucesso.`, "16px")}
